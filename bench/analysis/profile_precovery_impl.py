...
from precovery.search.run import precover_orbits_backend
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Profile backend-adapter precovery implementation on an experiment subset + stratified orbit sample."
    )
    p.add_argument(
        "--subset-dir",
        type=str,
        ...
        help="Subset directory (contains config.json + detections parquet reference).",
    )
    p.add_argument(
    ...
    if start_mjd is None or end_mjd is None:
        raise ValueError("--start-mjd and --end-mjd are required for backend search profiling")
    ...
    for oid, stratum, regime, orbit in zip(keep_id, keep_stratum, keep_regime, sub, strict=True):
        ...
        r = pa.table(
            {
                "orbit_id": pa.array([str(oid)], type=pa.large_string()),
                "regime": pa.array([str(regime)], type=pa.large_string()),
                "stratum": pa.array([str(stratum)], type=pa.large_string()),
                "n_accepted_detections": pa.array([int(len(run.accepted))], type=pa.int64()),
                "n_frames_skipped_limiting_mag": pa.array(
                    [int(run.n_frames_skipped_limiting_mag)], type=pa.int64()
                ),
            }
        )
        run_tables.append(r)
        ...
        print(
            ...
            f"n_accepted_detections={int(len(run.accepted))} "
            f"n_frames_skipped_limiting_mag={int(run.n_frames_skipped_limiting_mag)}"
        )
