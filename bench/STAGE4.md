...
Stage 4 detection filtering is implemented in `precovery/search/backend_pipeline.py` (Stage 4 orchestration) and
`precovery/search/detection_filter.py` (innovation-ellipse + magnitude residual gating primitives).
...
```bash
pdm run python -m bench.benchmarks.backend_benchmark --help
```
