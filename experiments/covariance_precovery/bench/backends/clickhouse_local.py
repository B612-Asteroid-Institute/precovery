...
@dataclass
class ClickHouseLocalBackend(BenchBackend):
    """
    ...
    def _create_temp_triples(self, *, client, triples: PredictedTriples) -> str:
        """
        Create a session-scoped temporary table for triples.
        """
        tmp = "tmp_triples"
        ...
        # Insert rows (small batches expected in benchmarks).
        data = list(
            zip(
        ...
        if data:
            client.insert(
                tmp,
                data,
                column_names=["orbit_id", "target_idx", "obscode", "exposure_mjd_mid_key_us", "healpixel"],
            )
        return tmp
    ...
    def fetch_candidates(
        ...
        *,
        ...
        return CandidateDetections.from_kwargs(
            ...
            observation_id=[str(r[2]) for r in rows],
            obstime_mjd_utc=[float(r[3]) for r in rows],
            ra_deg=[float(r[4]) for r in rows],
            dec_deg=[float(r[5]) for r in rows],
            ra_sigma_deg=[float(r[6]) for r in rows],
            dec_sigma_deg=[float(r[7]) for r in rows],
            mag=[None if r[8] is None else float(r[8]) for r in rows],
            mag_sigma=[None if r[9] is None else float(r[9]) for r in rows],
        )
